...
// If ctx is nil then context.Background() is used.
// If service is nil then a default service is created.
func GetAllCurrencyOK(t goatest.TInterface, ctx context.Context, service *goa.Service, ctrl app.CurrencyController, currencyURI string, creatorPubkey string, isCommitted *bool, target string) (http.ResponseWriter, *app.AnCurrency) {
	// Setup service
	var (
		logBuf bytes.Buffer
		resp   interface{}
	...
	var mt *app.AnCurrency
	if resp != nil {
		var ok bool
		mt, ok = resp.(*app.AnCurrency)
		if !ok {
			t.Fatalf("invalid response media: got %+v, expected instance of app.AnCurrency", resp)
		}
		_err = mt.Validate()
		if _err != nil {
			t.Errorf("invalid response media type: %s", _err)
