...
// OK sends a HTTP response with status code 200.
func (ctx *GetAccountsContext) OK(r *AnAccount) error {
	ctx.ResponseData.Header().Set("Content-Type", "application/vnd.an.account+json")
	return ctx.ResponseData.Service.Send(ctx.Context, 200, r)
}
...
// OK sends a HTTP response with status code 200.
func (ctx *GetAllCurrencyContext) OK(r *AnCurrency) error {
	ctx.ResponseData.Header().Set("Content-Type", "application/vnd.an.currency+json")
	return ctx.ResponseData.Service.Send(ctx.Context, 200, r)
}
